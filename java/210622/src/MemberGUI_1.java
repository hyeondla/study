...
public class MemberGUI_1 extends JFrame {
...
//				System.out.println(e.getKeyChar() + " : " + e.getKeyCode());
	...
	public void createNorthPanel() {
		...
		btnLogin.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(btnLogin.getText().equals("Login")) { // Login
					
					String dbUsername = tfDbUsername.getText();
					String dbPassword = new String(pfDbPassword.getPassword());
					
					if(dbUsername.length()==0) {
						JOptionPane.showMessageDialog(MemberGUI_1.this, "아이디 입력 필수", "로그인 요청", JOptionPane.WARNING_MESSAGE);
						tfDbUsername.requestFocus();
					} else if(dbPassword.length()==0) {
						JOptionPane.showMessageDialog(MemberGUI_1.this, "비밀번호 입력 필수", "로그인 요청", JOptionPane.WARNING_MESSAGE);					
	...
	public void createSouthPanel() {
		...
		ActionListener btnListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(!isLogin) { // 로그아웃 상태
					JOptionPane.showMessageDialog(MemberGUI_1.this, "로그인 필수", "로그인 요청", JOptionPane.WARNING_MESSAGE);
					tfDbUsername.requestFocus();
				} else { // 로그인 상태
	...
	public void insert() {
		...
		if(tfName.getText().length() == 0) {
			JOptionPane.showMessageDialog(MemberGUI_1.this, "이름 입력 필수", "입력 요청", JOptionPane.WARNING_MESSAGE);
			tfName.requestFocus();
			return;
		} else if(tfAge.getText().length() == 0) {
			JOptionPane.showMessageDialog(MemberGUI_1.this, "나이 입력 필수", "입력 요청", JOptionPane.WARNING_MESSAGE);
			tfAge.requestFocus();
			return;
		} else if(tfEmail1.getText().length() == 0) {
			JOptionPane.showMessageDialog(MemberGUI_1.this, "이메일 입력 필수", "입력 요청", JOptionPane.WARNING_MESSAGE);
			...
			JOptionPane.showMessageDialog(MemberGUI_1.this, "주민등록번호 입력 필수", "입력 요청", JOptionPane.WARNING_MESSAGE);
